...
if (isset($_POST['userSubmit'])){
    ...
    if (empty($errorMsg)){
        if(!empty($ID_Amenities)){
            ...
            $sql = "update [dbo].[Amenities] set Name_Amenities = ?,Descript = ?,URL_Image = ? where ID_Amenities = ?;";
            $query = $object->createConnection()->prepare($sql);
            $update = $query->execute(array($Name_Amenities,$Descript,$URL_Image,$ID_Amenities));
            if ($update){
            ...
            $sql = "insert into [dbo].[Amenities] (Name_Amenities,Descript,URL_Image) values(?,?,?);";
            $params = array(
                &$Name_Amenities,
                &$Descript,
                &$URL_Image,
